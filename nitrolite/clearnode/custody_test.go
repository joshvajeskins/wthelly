...
func createMockDepositedEvent(wallet common.Address, token string, amount *big.Int) *nitrolite.CustodyDeposited {
	return &nitrolite.CustodyDeposited{
		Wallet: wallet,
		Token:  common.HexToAddress(token),
		Amount: amount,
		Raw: types.Log{
			TxHash: newTestCommonHash("0xDepositTxHash"),
			Index:  0,
		},
	}
}

func TestHandleDepositedEvent(t *testing.T) {
	t.Run("Normal Deposit", func(t *testing.T) {
		custody, db, cleanup := setupMockCustody(t)
		defer cleanup()

		walletAddr := newTestCommonAddress("0xDepositWallet1")
		rawAmount := big.NewInt(1000000) // 1 USDC in raw (6 decimals)

		mockEvent := createMockDepositedEvent(walletAddr, tokenAddress, rawAmount)
		...
		custody.handleDeposited(logger, mockEvent)

		// Verify wallet balance was credited
		ledger := GetWalletLedger(db, walletAddr)
		walletAccountID := NewAccountID(walletAddr.Hex())
		balance, err := ledger.Balance(walletAccountID, "usdc")
		require.NoError(t, err)

		expectedDecimal := decimal.NewFromInt(1) // 1000000 / 10^6 = 1
		assert.True(t, expectedDecimal.Equal(balance),
			"Wallet balance should be %s, got %s", expectedDecimal, balance)

		// Verify ledger transaction was recorded
		var transactions []LedgerTransaction
		err = db.Where("to_account = ? AND asset_symbol = ?", walletAddr.Hex(), "usdc").Find(&transactions).Error
		require.NoError(t, err)
		assert.Len(t, transactions, 1)

		tx := transactions[0]
		assert.Equal(t, TransactionTypeDeposit, tx.Type)
		assert.Equal(t, "custody", tx.FromAccount)
		assert.Equal(t, walletAddr.Hex(), tx.ToAccount)
		assert.Equal(t, "usdc", tx.AssetSymbol)
		assert.True(t, expectedDecimal.Equal(tx.Amount))

		// Verify notification was sent
		assertNotifications(t, capturedNotifications, walletAddr.Hex(), 1)
	})

	t.Run("Large Deposit", func(t *testing.T) {
		custody, db, cleanup := setupMockCustody(t)
		defer cleanup()

		walletAddr := newTestCommonAddress("0xDepositWalletLarge")
		// 1 billion USDC in raw
		rawAmount := new(big.Int)
		rawAmount.SetString("1000000000000000", 10) // 10^15 = 1 billion USDC (6 decimals)
		...
		ledger := GetWalletLedger(db, walletAddr)
		balance, err := ledger.Balance(NewAccountID(walletAddr.Hex()), "usdc")
		require.NoError(t, err)

		expected := decimal.NewFromInt(1000000000) // 1 billion
		assert.True(t, expected.Equal(balance),
			"Wallet balance should be %s, got %s", expected, balance)
	})

	t.Run("Zero Amount Ignored", func(t *testing.T) {
		custody, db, cleanup := setupMockCustody(t)
		defer cleanup()

		walletAddr := newTestCommonAddress("0xDepositWalletZero")
		mockEvent := createMockDepositedEvent(walletAddr, tokenAddress, big.NewInt(0))
		...
		// No balance should be recorded
		ledger := GetWalletLedger(db, walletAddr)
		balance, err := ledger.Balance(NewAccountID(walletAddr.Hex()), "usdc")
		require.NoError(t, err)
		assert.True(t, balance.IsZero(), "Balance should be zero for zero deposit")

		// No notifications
		assert.Empty(t, capturedNotifications, "No notifications for zero deposit")
	})

	t.Run("Negative Amount Ignored", func(t *testing.T) {
		custody, db, cleanup := setupMockCustody(t)
		defer cleanup()

		walletAddr := newTestCommonAddress("0xDepositWalletNeg")
		mockEvent := createMockDepositedEvent(walletAddr, tokenAddress, big.NewInt(-1000000))
		...
		balance, err := ledger.Balance(NewAccountID(walletAddr.Hex()), "usdc")
		require.NoError(t, err)
		assert.True(t, balance.IsZero(), "Balance should be zero for negative deposit")
		assert.Empty(t, capturedNotifications, "No notifications for negative deposit")
	})
	...
	t.Run("Unknown Token Ignored", func(t *testing.T) {
		...
		walletAddr := newTestCommonAddress("0xDepositWalletUnknown")
		unknownToken := "0x0000000000000000000000000000000000000099"
		mockEvent := createMockDepositedEvent(walletAddr, unknownToken, big.NewInt(1000000))
		...
		balance, err := ledger.Balance(NewAccountID(walletAddr.Hex()), "usdc")
		require.NoError(t, err)
		assert.True(t, balance.IsZero(), "Balance should be zero for unknown token")
		assert.Empty(t, capturedNotifications, "No notifications for unknown token")
	})

	t.Run("Duplicate Event Idempotent", func(t *testing.T) {
		custody, db, cleanup := setupMockCustody(t)
		defer cleanup()

		walletAddr := newTestCommonAddress("0xDepositWalletDup")
		rawAmount := big.NewInt(5000000) // 5 USDC
		...
		expected := decimal.NewFromInt(5) // 5 USDC
		assert.True(t, expected.Equal(balance),
			"Balance should be %s (single deposit), got %s", expected, balance)

		// Only one ledger transaction
		var transactions []LedgerTransaction
		err = db.Where("to_account = ? AND asset_symbol = ?", walletAddr.Hex(), "usdc").Find(&transactions).Error
		require.NoError(t, err)
		assert.Len(t, transactions, 1, "Should have exactly 1 transaction despite duplicate event")
	})
	...
	t.Run("Multiple Deposits Accumulate", func(t *testing.T) {
		...
		walletAddr := newTestCommonAddress("0xDepositWalletMulti")

		logger := custody.logger

		// First deposit: 2 USDC
		ev1 := createMockDepositedEvent(walletAddr, tokenAddress, big.NewInt(2000000))
		ev1.Raw.TxHash = newTestCommonHash("0xDepositTx1")
		custody.handleDeposited(logger, ev1)

		// Second deposit: 3 USDC (different tx hash)
		ev2 := createMockDepositedEvent(walletAddr, tokenAddress, big.NewInt(3000000))
		ev2.Raw.TxHash = newTestCommonHash("0xDepositTx2")
		custody.handleDeposited(logger, ev2)

		ledger := GetWalletLedger(db, walletAddr)
		balance, err := ledger.Balance(NewAccountID(walletAddr.Hex()), "usdc")
		require.NoError(t, err)

		expected := decimal.NewFromInt(5) // 2 + 3 = 5 USDC
		assert.True(t, expected.Equal(balance),
			"Balance should be %s after two deposits, got %s", expected, balance)

		var transactions []LedgerTransaction
		err = db.Where("to_account = ? AND asset_symbol = ?", walletAddr.Hex(), "usdc").Find(&transactions).Error
		require.NoError(t, err)
		assert.Len(t, transactions, 2, "Should have 2 transactions for 2 deposits")
	})
}
