...
func (c *Custody) handleDeposited(logger Logger, ev *nitrolite.CustodyDeposited) {
	...
	wallet := ev.Wallet.Hex()
	tokenAddr := ev.Token.Hex()
	rawAmount := ev.Amount
	logger.Debug("parsed event", "wallet", wallet, "token", tokenAddr, "amount", rawAmount)

	if rawAmount.Sign() <= 0 {
		logger.Warn("ignoring deposit with non-positive amount", "amount", rawAmount)
		return
	}

	asset, ok := c.assetsCfg.GetAssetTokenByAddressAndChainID(tokenAddr, c.chainID)
	if !ok {
		logger.Warn("asset not found for deposit event", "token", tokenAddr, "chainID", c.chainID)
		return
	}
	...
	err := c.db.Transaction(func(tx *gorm.DB) error {
		...
		walletAddress := ev.Wallet
		walletAccountID := NewAccountID(wallet)
		...
		_, err := RecordLedgerTransaction(tx, TransactionTypeDeposit, NewAccountID("custody"), walletAccountID, asset.Symbol, amount)
		if err != nil {
			return err
		}

		logger.Info("handled deposited event", "wallet", wallet, "amount", amount, "asset", asset.Symbol)
		return nil
	})
	if errors.Is(err, ErrCustodyEventAlreadyProcessed) {
		return
	...
	c.wsNotifier.Notify(NewBalanceNotification(wallet, c.db))
}
